...
float	distort = 0.f;
int		View; // 0 = no texture, 1 = texture, 2 = distort

#define MS_PER_CYCLE 6550
...
Animate()
{
	...
	{
		distort = cos(((float)ms * 2) / (float)(MS_PER_CYCLE));
	}
	else
	{
		distort = sin(((float)ms * 2) / (float)(MS_PER_CYCLE));
	}
...
Display( )
{
	...
	else
	{
		...
		//distort texture by scaling face in and out
		glScalef((1.0 * distort), (1.0 * distort), 0.);
		//fprintf(stderr, "distort = %f\n", distort);
	...
	glLoadIdentity( );
	glColor3f( 1., 1., 1. );
	DoRasterString( 5., 5., 0., "Zach Schneider: Project 03" );
...
InitMenus( )
{
	...
	glutAddMenuEntry("Object", OBJECT);
